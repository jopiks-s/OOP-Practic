...
namespace _8
{
    public partial class Form1 : Form
    {
        ...
        {
            MessageBox.Show($"You choose index out of range: {m}", "Exception handling {IndexOutOfRange}", MessageBoxButtons.OK, MessageBoxIcon.Error);
        };

        handle_exception WrongTypeForArray= (m) =>
        {
            MessageBox.Show($"Attempt to add wrong type to array: {m}", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        };

        handle_exception WrongCast = (m) =>
        {
            MessageBox.Show($"Cannot cast between your input to: {m}", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        };

        handle_exception WrongDivideByZero = (m) =>
        {
            MessageBox.Show($"Error, you can't divide by zero", "Exception handling {DivideByZero}", MessageBoxButtons.OK, MessageBoxIcon.Error);
        };

        handle_exception Overflow = (m) =>
        {
            MessageBox.Show($"Cannot add more max element handle: {m}", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        };
        public Form1()
        {
            InitializeComponent();
