...
namespace _8
{
    public partial class Form1 : Form
    {
        ...
        {
            MessageBox.Show($"You choose index out of range: {m}");
        };

        handle_exception WrongTypeForArray= (m) =>
        {
            MessageBox.Show($"Attempt to add wrong type to array: {m}");
        };

        handle_exception WrongCast = (m) =>
        {
            MessageBox.Show($"Cannot cast between your input to: {m}");
        };

        handle_exception WrongDivideByZero = (m) =>
        {
            MessageBox.Show($"Error, you can't divide by zero");
        };

        handle_exception Overflow = (m) =>
        {
            MessageBox.Show($"Cannot add more max element handle: {m}");
        };
        public Form1()
        {
            InitializeComponent();
